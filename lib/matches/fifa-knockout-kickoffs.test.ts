...
describe("FIFA 2026 knockout kickoffs (country=CO)", () => {
  ...
  it("round of 32 Jun 28–30 matches FIFA calendar grouping", () => {
    ...
    expect(scheduleByDay["2026-06-30"]).toEqual([75, 77, 78]);
  });
  ...
  it("Mon 29 Jun has M76 (17:00) and M74 (20:30); M75 is Tue 30 Jun 01:00", () => {
    ...
    expect(m76.fifa_schedule_date).toBe("2026-06-29");
    expect(formatMatchTime(m76.kickoff_at)).toBe("17:00");
    expect(m74.fifa_schedule_date).toBe("2026-06-29");
    expect(formatMatchTime(m74.kickoff_at)).toBe("20:30");
    expect(m75.fifa_schedule_date).toBe("2026-06-30");
    expect(formatMatchTime(m75.kickoff_at)).toBe("01:00");
  });
});
